...
#         misc.print_module_summary(G, [z, c])
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, D=D, txt_recon=txt_recon, img_recon=img_recon, dataset_kwargs=args.dataset_kwargs, testset_kwargs=args.testset_kwargs,
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress, alpha=args.alpha, beta=args.beta)
        if rank == 0:
...
@click.command()
@click.pass_context
@click.option('network_pkl', '--lafite', help='Network pickle filename or URL', metavar='PATH', required=True)
...
@click.option('--alpha', help='Print optional information', type=float, default=True, metavar='FLOAT', show_default=True)
@click.option('--beta', help='Print optional information', type=float, default=True, metavar='FLOAT', show_default=True)

def calc_metrics(ctx, network_pkl, metrics, data, test_data, mirror, gpus, verbose, alpha, beta):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    # Validate arguments.
    args = dnnlib.EasyDict(metrics=metrics, num_gpus=gpus, network_pkl=network_pkl, verbose=verbose, alpha=alpha, beta=beta)
